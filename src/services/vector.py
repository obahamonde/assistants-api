...
class PineDantic(APIClient):
    ...
    @robust
    async def _upsert(self, *, vectors: List[Vector], metadata: List[MetaData]) -> int:
        payload = {
            "vectors": [
    ...
    async def _query(
        ...
        *,
        ...
        return [
            QueryResponse(score=match.score, text=str(match.metadata.get("text", "")))
    ...
    @robust
    async def upsert(self, *, text: str | list[str]) -> int:
        """
